...
AFlyCam::AFlyCam(PBE::NamedProperties const& properties)
	: APawn(properties)
{
	m_IsNetActor = true;
	m_name = "FlyCam";

	m_Model = AddComponent(g_Game->m_SkeletalModelSystem->CreateComponentFromFilepath("Proto", "Assets/Models/GLTF/dummy_platformer.gltf"));
	BlockPhysicsBodyCreateInfo info;
	info.m_Mass = 1.f;
	info.m_OBB = PBE::OBB3(PBE::Vec3(0.f), PBE::Vec3(0.85f,0.85f,2.f), PBE::Mat4::IDENTITY);
	info.m_Restitution = 0.5f;
	info.m_AngularDrag = 0.5f;
	info.m_Drag = 0.5f;
	info.m_Friction = 0.5f;
	info.m_HasGravity = true;
	info.m_CollisionGroup = 0x0;
...
void AFlyCam::OnPossessed(ActorUID actor)
{
		...
		action.m_Held = [this](PBE::InputActionValue const& value)
			{
				if(!m_noClip)
				{
					return; // No elevation when not in no-clip mode
				}
		...
		action.m_Started = [this](PBE::InputActionValue const& value)
			{
				...
				{
					collider->AddImpulse(PBE::Vec3(0.f, 0.f, 8.f));
					//collider->SetVelocity(PBE::Vec3(0.f, 0.f, 5.f));
				}
			};

		controller->m_ActionContext.BindAction("Jump", action);
...
void AFlyCam::OnUnpossessed()
{
	APawn::OnUnpossessed();
}

void AFlyCam::BeginFrame()
{
	APawn::BeginFrame();
	...
	if (TransformComponent* transform = m_Transform.GetAs<TransformComponent>())
	{
		if (BlockPhysicsBodyComponent* collider = m_Collider.GetAs<BlockPhysicsBodyComponent>())
		{
			...
			transform->SetPosition(collider->m_Position + PBE::Vec3(0.f,0.f,0.5f));
		}
	}
...
void AFlyCam::Update()
{
	...
	if (g_Engine->m_pInputSystem->WasKeyPressed(PBE::KEY_F1))
	{
	...
	if (g_Engine->m_pInputSystem->WasKeyPressed(PBE::KEY_F2))
	{
		if (m_noClip)
		{
			m_noClip = false;
			if (BlockPhysicsBodyComponent* collider = m_Collider.GetAs<BlockPhysicsBodyComponent>())
			{
				collider->m_HasGravity = true;
				collider->m_CollisionGroup = 0x1;
			}
		}
		else
		{
			...
			if (BlockPhysicsBodyComponent* collider = m_Collider.GetAs<BlockPhysicsBodyComponent>())
			{
				collider->m_HasGravity = false;
				collider->m_CollisionGroup = 0x0;
			}
		}
	}

	if (g_Engine->m_pInputSystem->WasKeyPressed(PBE::KEY_R))
	{
		PBE::Vec3 position = m_Transform.GetAs<TransformComponent>()->GetPosition();
		PBE::Vec3 forward = m_Transform.GetAs<TransformComponent>()->GetForward();
		PBE::RaycastResult3D result = g_BlockWorld->Raycast(position, forward, 100.f);
		...
		if (result.m_Hit)
		{
			...
			properties["Position"] = result.m_BlockPos + PBE::IntVec3(result.m_HitNormal);
			properties["Rotation"] = PBE::EulerAngles(0.f, 0.f, 0.f);

			SpawnActor(GetActorTypeFromName("ACharacter"), properties);
		}
	}
}

void AFlyCam::EndFrame()
{
	APawn::EndFrame();
	AController* controller = GetController();
	APlayerController* playerController = dynamic_cast<APlayerController*>(controller);
...
void AFlyCam::Translate(PBE::Vec3 const& translation)
{
	if (BlockPhysicsBodyComponent* collider = m_Collider.GetAs<BlockPhysicsBodyComponent>())
	{
...
void AFlyCam::Rotate(PBE::EulerAngles const& rotation)
{
	if (TransformComponent* transform = m_Transform.GetAs<TransformComponent>())
	{
